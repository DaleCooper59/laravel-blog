...
@section('content')
    ...
    <div class="p-1">
        <a class="flex pl-6" href="{{ route('articles.index') }}"> Articles
            <svg class="ml-1 text-Laurel_green h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
    ...
    <div class="max-w-2xl mx-auto py-4 px-8 bg-white shadow-lg rounded-lg my-20">
        <img @if ($article->picture === 'no') src="https://source.unsplash.com/random/" 
            @else
                src="{{ Storage::url($article->picture) }}" @endif alt="">
        ...
        <div class="shadow-shadowArticle p-5 text-center">
            ...
            <div class="flex justify-center md:justify-end -mt-16">
                ...
                <img class="w-20 h-20 object-cover rounded-full border-2 border-Laurel_green"
                    src="{{ $article->users->avatar === 'no-avatar' ? 'https://images.unsplash.com/photo-1499714608240-22fc6ad53fb2?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=334&q=80' : 'Storage::url($article->users->avatar)' }}">
    ...
    @else
        <p class="text-right pr-14">
            <a class="text-red-400" href="{{ route('login') }}">Connectez vous pour commenter</a>
            ou
