...
@section('h1')
    <div class="p-2">
        <h1 class=" text-3xl text-center text-Cambridge_blue"> Liste des commentaires pour l'article <span
                class="font-extrabold"> {{ $article->title }}</span></h1>
    </div>
                    ...
                    @if ($comment->approuved === 0)
                        <article
                            ...
                            <div>
                                ...
                                <img class="w-40  rounded m-1" @if ($users[0]->avatar === 'no-avatar') src="https://i.pravatar.cc/100?u={{ $comment->user_id }}"    
                    @else
                                src="{{ Storage::url($users[0]->avatar) }}" @endif alt="avatar">
                            </div>
